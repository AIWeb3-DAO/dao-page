...
export default function Features() {
  ...
  >
    <div className='my-4 flex flex-col space-y-2'>
         <h1  className='text-2ssxl md:text-4xl font-bold text-center '>Powered by Visionaries </h1>
         <p className='font-extralight text-center text-base md:text-2xl dark:text-neutral-200 py-3'>Powered by a Community of Visionaries and Innovators. <br />
         </p>
    </div>
        ...
        {features.map((item, i) =>  (
            <div key={i} className='border w-full md:w-[370px] p-4 rounded-xl hover:bg-muted'> 
             <div className={`p-3 rounded-full inline-flex items-center justify-center  bg-yellow-500/25 mb-4`}> 
                <item.icone className='w-7 h-7 text-yellow-600 hidden' />
                 <Image   src={item.logo} width={50} height={50} alt='supporter logo' className='w-14 h-14 rounded-full'   />
                </div> 
